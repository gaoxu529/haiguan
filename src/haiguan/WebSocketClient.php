...
class WebSocketClient
{
     ...
     */
    private function __construct($uri, $options = array())
    {
    ...
    static public function getInstance($socket_uri)
    {//声明一个getinstance()静态方法，用于检测是否有实例对象
        if (!self::$instance)
            self::$instance = new WebSocketClient($socket_uri);
    ...
    public function req($data)
    {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://customs.chinaport.gov.cn/ceb2grab/grab/realTimeDataUpload',
    ...
    public function req2($data)
    {
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://swapptest.singlewindow.cn/ceb2grab/grab/realTimeDataUpload',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_SSL_VERIFYPEER => false,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
